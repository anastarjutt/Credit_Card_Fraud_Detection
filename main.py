...
app = typer.Typer()
app.command(name='Train',help='Train a Model: [XGB, LGB, CAT, VOTING]',rich_help_panel='Main Command')

def run(
        model:str = typer.Option(...,help='Specify A Model: [XGB, LGB, CAT, VOTING]'),
    ...
    if model not in ['XGB','LGB','CAT','VOTING']:
        typer.secho('Invalid Model Name Choose One: XGB, LGB, CAT or VOTING',fg=typer.colors.RED)
        typer.Exit(1)
    ...
    try:
        if model == 'XGB':
            ...
            all_f1,all_roc,all_cm,all_clf,all_results = train.train_voting()
